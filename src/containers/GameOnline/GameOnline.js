...
class GameOnline extends React.Component {
  ...
  checkForWinner = squares => {
    ...
    for (let i = 0; i < possibleCombinations.length; i += 1) {
      const [a, b, c, d, e] = possibleCombinations[i];
      if (
        squares[a] &&
        squares[a] === squares[b] &&
        squares[a] === squares[c] &&
  ...
  onTieGame = () => {
    const title = 'Tie game!';
    const { gameChannel, pubnub } = this.props;
    Swal.fire({
      position: 'top',
      allowOutsideClick: false,
      title,
    ...
    return (
      <div className="game">
        ...
        <div className="game-info">
          ...
          <div>
            <p>Player X: {xScore} win</p>
          </div>

          <div>
            <p>Player O: {oScore} win </p>
          </div>
        </div>
      </div>
    );
...
export default withRouter(connect(mapStateToProps)(GameOnline));
